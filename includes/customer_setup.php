...
class MiniorangeSAMLIdpCustomer {
  ...
  function check_status($code){
	...
	function update_status() {
		...
		$url = MiniorangeSAMLIdpConstants::BASE_URL . '/moas/api/backupcode/updatestatus';
		$ch = curl_init($url);
	
		$customerKey = variable_get('miniorange_saml_idp_customer_id', '');
		$apiKey = variable_get('miniorange_saml_idp_customer_api_key', '');
		
		$lkey = variable_get('miniorange_saml_idp_sml_lk','');
		...
		$code = Utilities::decrypt($lkey, $key);
    ...
    function sueae($limit) 
    {
    
    $content = "Hello,<br><br>You have purchased license for Drupal SAML 2.0 IDP Plugin for <b>".$limit." users</b>. As number of users on your site have grown to more than ".$limit." users now. You should upgrade your license for miniOrange Drupal SAML 2.0 IDP plugin on your website.<br><br><a href='".MiniorangeSAMLIdpConstants::BASE_URL."/moas/login?redirectUrl=".MiniorangeSAMLIdpConstants::BASE_URL."/moas/initializepayment&requestOrigin=saml_sso_upgrade_instances_plan'>Click here</a> to upgrade the license to continue using our plugin.<br><br>Thanks,<br>miniOrange" ;
    $subject = 'Exceeded License Limit For No Of Users - Drupal SAML Single Sign-On Plugin';
    ...
    $fields = array(
          ...
          'email' => array(
              ...
              'fromName' => 'miniOrange',
              'toEmail' => variable_get('miniorange_saml_idp_customer_admin_email'),
              'toName' => $toEmail,
              'subject' => $subject,
              'content' => $content
          ),
	...
	function lexpirenotify($days) 
    {
    
    $content  = "Hello,<br><br>";
	$content .= $days!=0 ? "This email is to notify you that your 1 year license for WordPress SAML 2.0 IDP Plugin will expire in ".$days." days.The plugin will stop working after the licensed period expires."
				: "This email is to notify you that your 1 year license for WordPress SAML 2.0 IDP Plugin has expired. The plugin will stop working soon." ;
	$content .= "You will need to renew your license to continue using our plugin on your Drupal website.<br><br>Contact us at info@miniorange.com if you wish to renew your license.<br><br>Thanks,<br>miniOrange" ;
    $subject  = 'License Expired - Drupal SAML 2.0 IDP Plugin';
